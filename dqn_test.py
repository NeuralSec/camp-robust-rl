...
if __name__ == '__main__':
    ...
    if "baseline" in args.exp_name:
        ...
        exp_path = os.path.join(exp_path, f"wrapper_sigma={args.env_sigma}-lamda={args.lamda}-eval")
    ...
    elif 'freeway' in args.env_id or 'pong' in args.env_id:
    ...
    for i_episode in range(args.num_evals):
        ...
        while not done: 
            ...
            with torch.no_grad():
                q_table = torch.stack([dqn(state) for dqn in dqn_ensemble]).sum(0)
