...
def plot_gaussian():
	for j,sigma in enumerate([0.05, 0.1]):
		...
		attack_vals =  [torch.tensor(torch.load(path)).mean().item()/2. + .5]
		attack_sems = [sem(torch.tensor(torch.load(path))/2. + .5).item()]
		print(attack_sems)
		for attack_mag in attack_mags_nonzero:
			...
			for i,thresh in enumerate([0.0]):
				...
				cur_val = (torch.tensor( torch.load(data_path)).mean().item()/2. + .5)
				print("cur_val:", cur_val)
				if (attack_val is None or cur_val < attack_val):
					attack_val = cur_val
					attack_sem = (sem(torch.tensor( torch.load(data_path))/2. + .5)).item()
			attack_vals.append(attack_val)
			attack_sems.append(attack_sem)
			print("sem:", attack_sem)
...
def plot_camp():
	for j,sigma in enumerate([0.05, 0.1]):
		...
		for attack_mag in attack_mags_nonzero:
			...
			for i,thresh in enumerate([0.0]):
				if attack_type == "pgd":
					data_path=f"../exp/train_dqn_camp_final/{env_id}/wrapper_sigma={sigma}-lamda={lamda}-seed=0/checkpoints/{env_id}_restore.pth_" + \
						'evals_1000_attack_eps_' + str(attack_mag) + '_attack_step_count_multiplier_2' + '_attack_step_0.01_threshold_'+ str(thresh)+ '.pth'
				elif attack_type == "apgd":
...
plot_camp()
plt.legend()
plt.title('Pong (Single Round)', fontsize=18)
plt.xlim(0,.36)
plt.xlabel('Perturbation Budget', fontsize=14)
plt.ylim(0, 1.1)
plt.ylabel('Win Rate', fontsize=14)
plt.savefig(f'pong_1r_{attack_type}_threshold={q_gap}.pdf',dpi=400,bbox_inches='tight')
